...
export const TransactionsTable: React.FC<Props> = ({
  ...
  if (
    ...
    return (
      <div className="bg-[#141417] border border-zinc-800/80 rounded-2xl p-8 text-center text-zinc-400">
  ...
  return (
    <div className="bg-[#141417] border border-zinc-800/80 rounded-2xl overflow-hidden text-white shadow-xl">
      <div className="p-5 border-b border-zinc-800/80 flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-base">Riwayat Transaksi Ledger</h3>
          <p className="text-xs text-zinc-400 mt-0.5">Pencatatan ganda deterministik & reversal otomatis</p>
        </div>
        <span className="text-xs text-zinc-400 bg-zinc-800/80 px-2.5 py-1 rounded-full border border-zinc-700/50">
          {totalCount} mutasi
        </span>
      </div>

      {/* Filter and Search Controls */}
      <div className="p-4 sm:p-5 border-b border-zinc-800/80 flex flex-col lg:flex-row gap-3 justify-between items-stretch lg:items-center bg-zinc-950/20">
        ...
        <div className="flex flex-wrap gap-1.5 p-1 bg-zinc-950/40 border border-zinc-800/80 rounded-xl self-start lg:self-auto shrink-0">
      ...
      <div data-testid="transactions-mobile-cards" className="block sm:hidden divide-y divide-zinc-800/60">
        {displayTransactions.length === 0 ? (
          <div className="py-10 px-4 text-center text-zinc-400 text-sm">
            {hasActiveFilters
            ...
            {displayTransactions.map((tx) => {
              ...
              return (
                <motion.div
                  ...
                  className="p-4 hover:bg-zinc-800/50 transition-colors flex flex-col gap-3"
                >
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1 border-t border-zinc-800/40">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {category && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-zinc-800/80 text-zinc-300 border border-zinc-700/50">
                          {category.name}
                        </span>
                      )}
                      <Chip
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 z-10 backdrop-blur-md bg-[#0c0c0e]/85 text-xs font-semibold text-zinc-400 border-b border-zinc-800/80">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-800/60 text-sm">
            {displayTransactions.length === 0 ? (
              <tr>
                <td colSpan={3} className="py-10 px-4 text-center text-zinc-400 text-sm">
                  {hasActiveFilters
                    ? 'Tidak ada transaksi yang sesuai dengan filter pencarian.'
                    : 'Belum ada transaksi.'}
                </td>
                      ...
                      <td className="py-3.5 px-4 sm:px-6">
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-0.5 text-xs text-zinc-400">
                              <span>{dateStr}</span>
                              <Chip size="sm" variant="flat" color={channelChipProps.color} className="text-[10px] h-5 px-1">
                                {channelChipProps.label}
                              </Chip>
      ...
      <div className="p-4 sm:p-5 border-t border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-zinc-400 bg-zinc-950/20">
        <span>
      ...
      >
        <ModalContent>
          {() => (
            <>
              <ModalHeader>
                <h3 className="font-semibold text-lg text-zinc-100">Konfirmasi Hapus Transaksi</h3>
              </ModalHeader>
              <ModalBody>
